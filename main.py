...
# 🟢 Important: Use Python's datetime, not nepali_datetime
import datetime
...
def log_checkin(name):
    now_ad = datetime.datetime.now()  # ✅ Python datetime
    now_bs = nep_date.from_datetime_date(now_ad.date())  # ✅ Convert to Nepali date
    with open(LOG_PATH, "a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow([name, now_bs.strftime("%Y-%m-%d"), now_ad.strftime("%H:%M:%S")])
